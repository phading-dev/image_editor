...
export class OvalMaskSelectionTool {
  ...
  private handlePointerUpOrCancel = (e: PointerEvent): void => {
    ...
    if (this.currentRect.width > 1 && this.currentRect.height > 1) {
      ...
      const h = rect.x + rect.width / 2;
      const k = rect.y + rect.height / 2;
      ...
      for (let y = rect.y; y < rect.y + rect.height; y++) {
        for (let x = rect.x; x < rect.x + rect.width; x++) {
          if (x >= 0 && x < mask.width && y >= 0 && y < mask.height) {
            const dx = x - h;
            const dy = y - k;
            const val = (dx * dx) / a2 + (dy * dy) / b2;

            // Apply anti-aliasing
            // Gradient magnitude estimation: |grad f| = 2 * sqrt((dx/a^2)^2 + (dy/b^2)^2)
            // Distance to boundary approx: (1 - val) / |grad f|

            // We iterate over the bounding box. If val is significantly larger than 1, we are outside.
            // Using 1.5 as a safe upper bound to include the anti-aliased edge.
            if (val <= 1.5) {
              const gradLen =
                2 * Math.sqrt((dx * dx) / (a2 * a2) + (dy * dy) / (b2 * b2));
              const dist = (1 - val) / gradLen;
              // Clamp distance to [-0.5, 0.5] pixel range for edge coverage
